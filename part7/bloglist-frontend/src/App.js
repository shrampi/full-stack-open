...
import { addNotification, removeNotification } from './reducers/notification';
...
const App = () => {
  ...
  useEffect(() => {
    blogService
      .getAll()
      .then((blogs) => setBlogs(blogs.sort(blogsLikesComparison)));
  }, []);
    ...
    blogService
      ...
      .then(() => {
        const blogToUpdate = blogs.find(
          (b) => b._id.toString() === updatedBlog._id.toString()
        );
        blogToUpdate.likes += 1;
        setBlogs(
          blogs
            .map((b) =>
              b._id.toString() === blogToUpdate._id.toString()
                ? blogToUpdate
                : b
            )
            .sort(blogsLikesComparison)
        );
        notify(`liked ${blogToUpdate.title}`)
      })
      .catch((error) => {
  ...
  const removeBlog = (blog) => {
    if (window.confirm(`Do you really want to delete ${blog.title}?`)) {
      blogService
        ...
        .then(() => {
          setBlogs(
            blogs.filter((b) => b._id.toString() !== blog._id.toString())
          );
        })
        .catch((error) => {
