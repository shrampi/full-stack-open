...
beforeEach(() => {
  ...
  container = render(
    <Blog blog={blog} incrementLikes={incrementLikes} removeBlog={removeBlog} />
  ).container;
});

test('blog renders title but not author/url/likes by default', () => {
  expect(container.querySelector('.blogTitle')).not.toHaveStyle(
    'display: none'
  );
...
test('blog renders details when button is clicked', async () => {
  ...
  expect(container.querySelector('.blogDetails')).not.toHaveStyle(
    'display: none'
  );
});
